...
public class MaxisXlsWriter extends XLSWriter {
	Logger log = Logger.getLogger(this.getClass());	
	public MaxisXlsWriter() {
		super();
	}
	...
	private void writeRevShareRptSheet(XSSFSheet s,HashMap<String, Collection<ReportDataDTO>> hmReport, boolean includeCP) throws Exception {
	    ...
	    r = s.createRow(rownum++);
		iCount = 0;
		XLSUtil.addCellToRow(r, iCount++, "Free Item Share (Network Aware Apps)", csBody);
		XLSUtil.addCellsToRow(r, iCount++, 10, "", csBody);
		log.debug("Cell No : "+ (iCount+10));
		XLSUtil.addCellToRow(r, iCount+9, (iNetworkAwareApps*0.5), csAmt);
		...
		log.debug("Maxis REPORT WRITING DONE");
	}
