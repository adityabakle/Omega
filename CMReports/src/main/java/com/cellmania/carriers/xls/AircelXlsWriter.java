...
public class AircelXlsWriter extends XLSWriter {
	Logger log = Logger.getLogger(this.getClass());
	
	public AircelXlsWriter() {
		super();
	}
	...
	public String createFreeItemInvoiceReport(
		...
		log.debug("In writeFreeItemInvoiceRpt Aircel");
		String fileName = Utility.getFileName(getReqReport());
		...
		try {
				...
				if (colRpt != null) {
					for (ReportDataDTO rDto : colRpt) {
						if (rDto.getCircleName().equals("Rajasthan")) {
							...
							rownum_sRI++;
							if (rownum_sRI > XLSWriter._XLSX_MAX_ROW_COUNT-1) {
			...
			log.error("Exception writeFreeItemInvoiceRpt Aircel XLS : " + e);
			e.printStackTrace();
		}
		return fileName;
	}
	...
	public String createPaidItemInvoiceRpt(HashMap<String, Collection<ReportDataDTO>> hmReport){
		log.debug("In writePaidItemInvoiceRpt Aircel");
		String fileName = Utility.getFileName(getReqReport());
		...
		try {
			...
			log.error("Exception writing Aircel XLS : " + e);
			e.printStackTrace();
		}
		return fileName;		
	}
	...
	private void creatFreeSummary(XSSFSheet s, double totSal, int row) {
		...
		for(int j=0; j<11; j++) {
			...
			log.error("Erro while expanding Column in Report Invoice for Aircel",e);
		}
