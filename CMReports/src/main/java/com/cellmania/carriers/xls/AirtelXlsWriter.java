...
public class AirtelXlsWriter extends XLSWriter {
	Logger log = Logger.getLogger(this.getClass());
	
	public AirtelXlsWriter() {
		super();
	}
	...
	public String createFreeItemInvoiceReport(
		...
		log.debug("In writeFreeItemInvoiceRpt Airtel");
		String fileName = Utility.getFileName(getReqReport());
		...
		try {
			...
			log.error("Exception writeFreeItemInvoiceRpt Airtel XLS : " + e);
			e.printStackTrace();
		}
		return fileName;
	}
	...
	public String createPaidItemInvoiceRpt(HashMap<String, Collection<ReportDataDTO>> hmReport){
		log.debug("In writePaidItemInvoiceRpt Airtel");
		String fileName = Utility.getFileName(getReqReport());
		...
		try {
			...
			log.error("Exception writing Airtel XLS : " + e);
			e.printStackTrace();
		}
		return fileName;		
	}
	...
	private void creatFreeSummary(XSSFSheet s, double totSal, int row) {
		...
		for(int j=0; j<11; j++) {
			...
			log.error("Erro while expanding Column in Report Invoice for Airtel",e);
		}
