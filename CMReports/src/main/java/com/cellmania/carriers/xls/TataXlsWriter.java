...
public class TataXlsWriter extends XLSWriter {
	Logger log = Logger.getLogger(this.getClass());	
	public TataXlsWriter() {
		super();
	}
	...
	private void writeRevShareRptSheet(XSSFSheet s,HashMap<String, Collection<ReportDataDTO>> hmReport, boolean includeCP) throws Exception {
	    ...
	    XLSUtil.addCellToRow(r, iCount++, "Net sales", csHeader);
	    ...
	    XLSUtil.addCellToRow(r, iCount++, "NWA\nFree Apps", csHeader);
	    XLSUtil.addCellToRow(r, iCount++, "NWA Free Apps\nDownload Charge", csHeader);
		...
		double dTotalSales = 0.0,dTotalNetSales=0.0,dTotalCS=0.0,dTotalCM=0.0,dTotalCP=0.0,dTotalCI=0.0, nwaDownloadCharge=0.0; 
		...
		for(ReportDataDTO rpt : colRpt) {
			...
			nwaDownloadCharge += rpt.getZeroPriceSales();
		...
		XLSUtil.addCellToRow(r, iCount++, nwaDownloadCharge, csAmt_Bold);
	    ...
	    for(ReportDataDTO rpt : colRpt) {
			...
			XLSUtil.addCellToRow(r, iCount++, rpt.getNoOfFreeNWApps(), csBody);
			iNetworkAwareApps+=rpt.getNoOfFreeNWApps();
			XLSUtil.addCellToRow(r, iCount++, rpt.getZeroPriceSales(), csBody);
			nwaDownloadCharge += rpt.getZeroPriceSales();
			XLSUtil.addCellToRow(r, iCount++, rpt.getCarrierShare(), csAmt);
			dTotalCS+=rpt.getCarrierShare();
			if(includeCP || "direct".equals(rpt.getDirectFlag())){
	    ...
	    r = s.createRow(rownum++);
		iCount = 0;
		s.addMergedRegion(new CellRangeAddress(rownum-1,rownum-1,0,4));
		XLSUtil.addCellToRow(r, iCount++, "Network Aware Apps (NWA) Download Charge", csBody);
		XLSUtil.addCellsToRow(r, iCount++, 11, "", csBody);
		log.debug("Cell No : "+ (iCount+11));
		XLSUtil.addCellToRow(r, iCount+10, nwaDownloadCharge, csAmt);
		dTotalCI += nwaDownloadCharge;
