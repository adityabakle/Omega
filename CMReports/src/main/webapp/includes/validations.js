...
function trimString(s) {
	s = s.replace(/(^\s*)|(\s*$)/gi,""); //remove only leading or trailing spaces , so commented below lines
	//s = s.replace(/[ ]{2,}/gi," ");
	//s = s.replace(/\n /,"\n");
	return s;
}
...
{   var secondArg = defaultEmptyOK;
    ...
    return (isSignedInteger(s, secondArg)
         && ( (isEmptyStr(s) && secondArg)  || (parseInt (s) > 0) ) );
}
...
{   // Next line is needed on NN3 to avoid "undefined is not a number" error
    ...
    if ((emptyOK == true) && (isEmptyStr(theField.value))) return true;
...
{   // Next line is needed on NN3 to avoid "undefined is not a number" error
    ...
    if ( (OKtoOmitDay == true) && isEmptyStr(dayField.value) ) return true;
